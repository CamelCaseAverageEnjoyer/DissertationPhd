...
def talk_decision(cnd=True):
    from random import choice
    if cnd:
        talk_aloud(choice(['Работай нахуй!', 'Вот хуй!', 'Ща порву тебя нахуй!', 'Йобушки воробушки', 'Пиздец',
...
def talk_notice(cnd=True):
    from random import choice
    if cnd:
        talk_aloud(choice(['Вон вон вон он сука!', 'Мне показалось?', 'Там что-то есть', 'Нет, мне не кажется']))

def talk_success(cnd: bool = True) -> None:
    from random import choice
    if cnd:
        talk_aloud(choice(['Этот перец не такой кривой как я думала', 'Охуеть!', 'Неужели я это вижу?']))
